...
public class GreedyAgentOpt implements MSWAgent
{
	...
	*/
	public GreedyAgentOpt()
	{
		this.name = "G0PT by abraram";
		setGreedyAgent();
	}
	...
	*/
	public GreedyAgentOpt(String providedName)
	{
	 ...
	 */
 	private List<Card> findLargestSuitInHand()
 	{
		...
		if(ls.size() == clubs.size()){
			if(compare(ls.get(ls.size()-1), clubs.get(ls.size()-1)) > 0){ls = this.clubs;}
		}
		if(ls.size() == diamonds.size()){
			if(compare(ls.get(ls.size()-1), diamonds.get(ls.size()-1)) > 0){ls = this.diamonds;}
		}
		if(ls.size() == hearts.size()){
			if(compare(ls.get(ls.size()-1), hearts.get(ls.size()-1)) > 0){ls = this.hearts;}
		}
