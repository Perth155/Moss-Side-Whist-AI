...
/**
    GreedyAdvanced is a rational decison making agent for the game Moss Side Whist.
...
public class GreedyAdvanced implements MSWAgent{
    ...
    private String agentName = "Greedy Advanced";
    ...
    /**
     * Card Set Without Suit:
     * This method returns all of the cards that do not exist for a particular set.
     * @param suit The card suit we will be removing from the list.
     * @param hand The list of cards to search through.
     * @return A list of cards that does not contain the suit.
     */
    public List<Card> cardSetWithoutSuit(Suit suit, List<Card> hand) {
        List<Card> availableCards = new ArrayList<Card>();
        int i = 0;
        while (i < hand.size()) { //Iterate over the list and add any cards if found
            if ((hand.get(i).suit != suit)) {
    ...
    /**
     * Count Cards:
     * This method takes a set of cards and counts how many cards of a particular suit are in the set.
     * If no card of the suit can be found in the hand, a value of zero is returned..
     * @param suit The card suit we are searching for in the list
     * @param hand The list of cards to search through.
     * @return An integer with the total number of cards of suit.
     */
    public int countCards(Suit suit, List<Card> hand) {
        int count = 0;
        int i = 0;
        while (i < hand.size()) { //Iterate over the list and add any cards if found
            if ((hand.get(i).suit == suit)) {
     ...
     * Uses a more advanced heuristic to try beat the other cards.
     * The agent first makes a comparison between the spades of cards in our hand relative to the opponents.
     * If we have more spades in our hand than the other players, we play spades first to flush out the opponents cards.
     * If this is not the case, lead with the strongest ranked cards we have that are not spades.
     * @return The final card
     */
    public Card firstPositionPlay() {
        int agentSpades = countCards(Suit.SPADES, hand);
        int enemySpades = countCards(Suit.SPADES, storage.retrieveDeck());

        // Compare if our hand has more spades than opponents
        if (enemySpades != 0 && (agentSpades >= enemySpades)) {
            Card[] ranked = rankCards(cardSet(Suit.SPADES, hand), Suit.SPADES);
            return hand.remove(cardIndex(ranked[ranked.length - 1]));
        } else {
            Card[] ranked = rankCards(cardSetWithoutSuit(Suit.SPADES, hand), null);
...
class cardStorage {
    ...
    // This stack contains all the cards in the deck for players other than the agent
    ...
    // Store the names of each player so we know which place stack to store and retrieve the cards
    ...
    // Publish the deck
    public void publishDeck(List<Card> hand) {
        for (int i = 0; i < hand.size(); i++) {
            deck.remove(hand.get(i));
        }
    }

    // Retrieve the deck of cards that are yet to be played
    ...
    public void storeCard(String playerName, Card card) {
        ...
        // As we are storing a card in our played cards list, we want to remove it from the active deck
        if (playerName != "Greedy Advanced") {
            deck.remove(card);
        }
    }

    // Receives the scoreboard and documents this for future access
    public void storeScore(Map<String, Integer> scoreboard) {
        for (Map.Entry<String, Integer> entry : scoreboard.entrySet()) {
            if (entry.getKey() == p1Name) {
                p1Score = entry.getValue() - p1Tally;
                p1Tally = entry.getValue();
            } else if (entry.getKey() == p2Name) {
                p2Score = entry.getValue() - p2Tally;
                p2Tally = entry.getValue();
            } else {
                p3Score = entry.getValue() - p3Tally;
                p3Tally = entry.getValue();
            }
        }
    }

    // Retrieve round score for a player
    ...
    public Map<String, Integer> retrieveScores() {
        Map<String, Integer> scores = new HashMap<String, Integer>();
        scores.put(p1Name, p1Score);
        scores.put(p2Name, p2Score);
        scores.put(p3Name, p3Score);
        return scores;
    }
